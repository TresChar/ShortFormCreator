...
#from utils import remove_profanity
#from profanity_removal import 
...
from no_profanity import profanity


def remove_profanity(result,
                     family_friendly = True
                     ):
    profanity.load_censor_words()
    result = profanity.censor(result,censor_word="bleep")
    return result
def sanitize_text(text: str) -> str:
    ...
    ##need to add a variable in config and config template that allows someone to input a censor word.It is set one ahead of this below func
    #REMOVE ANY FILTERED WORDS
    if settings.config["settings"]["tts"]["family_friendly"]:
        result=remove_profanity(result, family_friendly = True)
