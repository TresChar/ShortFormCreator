...
def save_text_to_mp3(reddit_obj,text_object) -> Tuple[int, int]:
    ...
    if str(voice).casefold() in map(lambda _: _.casefold(), TTSProviders):
        print(f"save_text_tomp3{get_case_insensitive_key_value(TTSProviders, voice)}")
        text_to_mp3 = TTSEngine(get_case_insensitive_key_value(TTSProviders, voice), reddit_obj,text_object)
    else:
        while True:
        ...
        text_to_mp3 = TTSEngine(get_case_insensitive_key_value(TTSProviders, choice), reddit_obj,text_object)
