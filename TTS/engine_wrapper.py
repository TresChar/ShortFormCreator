...
class TTSEngine:
    ...
    def run(self) -> Tuple[int, int]:
        ...
        self.add_periods()
        self.call_tts("title", process_text(self.reddit_object["thread_title"]),word=False)
        ...
        if settings.config["settings"]["storymode"]:
            if settings.config["settings"]["storymodemethod"] == 0:
                if len(self.reddit_object["thread_post"]) > self.tts_module.max_chars:
                    ...
                    self.call_tts("postaudio", process_text(self.reddit_object["thread_post"]),word=False)
                ...
                for idx_word, text in track(enumerate(texts), "Rendering Image"):
                    ...
                    for word in split_text:
                        ...
                        #print(process_text(word))
                        
                        #if(word != '"'):                            
                        #maybe this should be before
                        #print(word)
                        self.call_tts(f"word{idx_word}-{word_count}", process_text(word),word=True)
            ...
            for idx, comment in track(enumerate(self.reddit_object["comments"]), "Saving..."):
                ...
                if (
                    ...
                    self.call_tts(f"{idx}", process_text(comment["comment_body"]),word=False)

        print_substep("Saved Text to MP3 files successfully.", style="bold green")
        return self.length, idx, self.text_object
    ...
    def split_post(self, text: str, idx):
        ...
        for idy, text_cut in enumerate(split_text):
            ...
            if not newtext or newtext.isspace():
                ...
                self.call_tts(f"{idx}-{idy}.part", newtext,word=False)
    ...
    def call_tts(self, filename: str, text: str,word: bool):
        #print(f"text in call_tts={text}")
        if(text!="" ):
