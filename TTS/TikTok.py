...
class TikTok:
    ...
    def run(self, text: str, filepath: str, random_voice: bool = False,word: bool= None):
        ...
        #print(f"text in TikTok.py={text}")
        data = self.get_voices(voice=voice, text=text,word=word)
    ...
    def get_voices(self, text: str, voice: Optional[str] = None,word: bool = None) -> dict:
        """If voice is not passed, the API will try to use the most fitting voice"""
        # sanitize text
        text = text.replace("+", "plus").replace("&", "and").replace("r/", "").replace(",","")
